...
void *__dup_pagetable(pid_t pid, uintptr_t mount_point)
{
    ...
    for (size_t i = 0; i < PG_MAX_ENTRIES - 1; i++)
    {
        ...
        for (size_t j = 0; j < PG_MAX_ENTRIES; j++)
        {
            ...
            // FIXME: 根据 mm_region
            // 将读共享的页（如堆）标为只读，而私有的页（如栈），则复制；而写共享的页则无需更改flags
            if (va >= KSTACK_START)
            {
                void *ppa = vmm_dup_page(pid, (void *)PG_ENTRY_ADDR(ppte));
...
void dup_proc()
{
    ...
    /*
        FIXME: Problematic! It should mount the page table of process then copy
       it. The current implementation copy the CURRENTLY loaded pgt. However,
       dup_pagetable is designed to copy current loaded pgt.

    */

    void *mnt_pt = vmm_mount_pd(__current->page_table);

    void *pg = __dup_pagetable(pid, (uintptr_t)mnt_pt);

    vmm_unmount_pd();

    struct proc_info pcb =
        (struct proc_info){.created = clock_systime(),
                           .pid = pid,
                           .mm = __current->mm,
                           .page_table = pg,
