...
void *__dup_pagetable(pid_t pid, uintptr_t mount_point)
{
    ...
    for (size_t i = 0; i < PG_MAX_ENTRIES - 1; i++)
    {
        ...
        for (size_t j = 0; j < PG_MAX_ENTRIES; j++)
        {
            ...
            pmm_ref_page(pid, (void *)(pte & ~0xfff));
            pt->entry[j] = pte;
        }
...
void dup_proc()
{
    ...
    struct proc_info pcb = (struct proc_info){.created = clock_systime(),
                                              .pid = pid,
                                              .mm = __current->mm,
    ...
    if (__current->mm.regions)
    {
        ...
        llist_for_each(pos, n, &__current->mm.regions->head, head)
        {
            ...
            {
                x86_pte_t *curproc =
                    &((x86_page_table *)(PD_MOUNT_1 | ((i & 0xffc00) << 2)))
                         ->entry[i & 0x3ff];
                x86_pte_t *newproc =
                    &((x86_page_table *)(PD_MOUNT_2 | ((i & 0xffc00) << 2)))
                         ->entry[i & 0x3ff];
