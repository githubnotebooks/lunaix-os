...
void *__dup_pagetable(pid_t pid, uintptr_t mount_point)
{
    ...
    x86_page_table *ptd =
        vmm_fmap_page(pid, (void *)PG_MOUNT_1, ptd_pp, PG_PREM_RW);
    ...
    for (size_t i = 0; i < PG_MAX_ENTRIES - 1; i++)
    {
        ...
        x86_page_table *pt =
            vmm_fmap_page(pid, (void *)PG_MOUNT_2, pt_pp, PG_PREM_RW);

        for (size_t j = 0; j < PG_MAX_ENTRIES; j++)
        {
            x86_pte_t pte = ppt->entry[j];
            pmm_ref_page(pid, (void *)PG_ENTRY_ADDR(pte));
            pt->entry[j] = pte;
        }
...
void __del_pagetable(pid_t pid, uintptr_t mount_point)
{
    ...
    for (size_t i = 0; i < PG_MAX_ENTRIES - 1; i++)
    {
        ...
        for (size_t j = 0; j < PG_MAX_ENTRIES; j++)
        {
            ...
            if ((pte & PG_PRESENT))
            {
                pmm_free_page(pid, (void *)PG_ENTRY_ADDR(pte));
            }
        }
        // free the L2 page table
        pmm_free_page(pid, (void *)PG_ENTRY_ADDR(ptde));
    }
    // free the L1 directory
    pmm_free_page(pid, (void *)PG_ENTRY_ADDR(pptd->entry[PG_MAX_ENTRIES - 1]));
}
...
void setup_proc_mem(struct proc_info *proc, uintptr_t usedMnt)
{
    ...
    for (size_t i = KSTACK_START >> 12; i <= KSTACK_TOP >> 12; i++)
    {
        ...
        */
        ...
        x86_pte_t p = *ppte;
        void *ppa = vmm_dup_page(pid, (void *)PG_ENTRY_ADDR(p));
