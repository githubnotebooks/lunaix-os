...
static inline void _set_idt_entry(uint32_t vector, uint16_t seg_selector,
                                  void (*isr)(), uint8_t dpl, uint8_t type)
{
...
void _set_idt_intr_entry(uint32_t vector, uint16_t seg_selector, void (*isr)(),
                         uint8_t dpl)
{
    _set_idt_entry(vector, seg_selector, isr, dpl, IDT_INTERRUPT);
}

void _set_idt_trap_entry(uint32_t vector, uint16_t seg_selector, void (*isr)(),
                         uint8_t dpl)
{
