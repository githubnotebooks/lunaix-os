...
void intr_routine_page_fault(const isr_param *param)
{
    ...
    if (*pte & PG_PRESENT)
    {
            ...
            (REGION_RSHARED | REGION_READ))
        {
            ...
            uintptr_t pa = (uintptr_t)vmm_dup_page(__current->pid,
                                                   (void *)PG_ENTRY_ADDR(*pte));
            pmm_free_page(__current->pid, (void *)(*pte & ~0xFFF));
    ...
    // a writable page, not present, pte attr is not null and no indication of
    // cached page -> a new page need to be alloc
    if ((hit_region->attr & REGION_WRITE) && (*pte & 0xfff) && !loc)
    {
        uintptr_t pa = (uintptr_t)pmm_alloc_page(__current->pid, 0);
