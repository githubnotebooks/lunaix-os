...
int dmm_init(heap_context_t *heap)
{
    ...
    return vmm_alloc_page(__current->pid, heap->brk, NULL, PG_PREM_RW, 0) !=
           NULL;
}
...
void *lxsbrk(heap_context_t *heap, size_t size)
{
    ...
    void *next = (char *)current_brk + ROUNDUP(size, BOUNDARY);
    ...
    if (diff)
    {
        ...
        if (!vmm_alloc_pages(__current->pid,
                             (void *)(PG_ALIGN(current_brk) + PG_SIZE), diff,
                             PG_PREM_RW, 0))
        {
