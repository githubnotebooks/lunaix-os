...
ptd_t *get_pd()
{
    ...
    __asm__("movl %%cr3, %0\n"
            ...
            : "=r"(pd));
#endif
    return reinterpret_cast<ptd_t *>(P2V(pd));
}
...
ptd_t *vmm_init_pd()
{
    auto &&dir = static_cast<ptd_t *>(pmm_alloc_page());
...
void *vmm_map_page(void *va, void *pa, pt_attr dattr, pt_attr tattr)
{
    ...
    ptd_t *ptd = reinterpret_cast<ptd_t *>(PTD_BASE_VADDR);
    ...
    while (pde && pd_offset < 1024)
    {
        if (pt_offset == 1024)
        {
            pd_offset++;
            pt_offset = 0;
            pde = reinterpret_cast<ptd_t *>(ptd[pd_offset]);
    ...
    // 页目录有空位，需要开辟一个新的 PDE
    uint8_t *new_pt_pa = static_cast<uint8_t *>(pmm_alloc_page());
...
void vmm_unmap_page(void *vpn)
{
    ...
    ptd_t *self_pde = reinterpret_cast<ptd_t *>(PTD_BASE_VADDR);
    ...
    if (pde)
    {
        ...
        if (IS_CACHED(pte) && pmm_free_page(reinterpret_cast<void *>(pte)))
        {
// 刷新TLB
#ifdef __ARCH_IA32
