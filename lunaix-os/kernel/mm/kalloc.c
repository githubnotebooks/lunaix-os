...
void lx_free_internal(void *ptr);
...
int kalloc_init()
{
    ...
    SW((char *)__kalloc_kheap.start + WSIZE, PACK(0, M_ALLOCATED));
    __kalloc_kheap.brk = (char *)__kalloc_kheap.brk + WSIZE;
...
void *lx_grow_heap(heap_context_t *heap, size_t sz)
{
    ...
    return (void *)coalesce((uint8_t *)start);
}
