...
#pragma endregion
    ...
    // 初始化VGA
    tty_init(reinterpret_cast<void *>(VGA_BUFFER_PADDR));
...
#pragma endregion
    ...
    {
        vmm_map_page(reinterpret_cast<void *>(VGA_BUFFER_VADDR + (i << 12)),
                     reinterpret_cast<void *>(VGA_BUFFER_PADDR + (i << 12)),
                     PG_PREM_RW, PG_PREM_RW);
    }

    // 更新VGA缓冲区位置至虚拟地址
    tty_set_buffer(reinterpret_cast<void *>(VGA_BUFFER_VADDR));
    ...
    {
        vmm_alloc_page(reinterpret_cast<void *>(K_STACK_START + (i << 12)),
                       PG_PREM_RW, PG_PREM_RW);
    }
...
extern "C" void _kernel_post_init()
{
    ...
    {
        vmm_unmap_page(reinterpret_cast<void *>(i << 12));
    }
