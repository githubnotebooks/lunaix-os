...
void _kernel_init()
{
    ...
    {
        vmm_alloc_page(KERNEL_PID, (void *)(KSTACK_START + (i << PG_SIZE_BITS)),
                       NULL, PG_PREM_RW, 0);
    }
...
void spawn_lxinit()
{
    ...
    kinit.parent = -1;
    kinit.pid = 1;
    kinit.intr_ctx = (isr_param){.registers.esp = (reg32)kstack,
                                 .cs = KCODE_SEG,
                                 .eip = (unsigned int)_kernel_post_init,
                                 .ss = KDATA_SEG,
                                 .eflags = cpu_reflags()};
...
void _kernel_post_init()
{
    ...
    pmm_mark_page_occupied(KERNEL_PID, FLOOR(__APIC_BASE_PADDR, PG_SIZE_BITS),
                           0);
    pmm_mark_page_occupied(KERNEL_PID, FLOOR(ioapic_addr, PG_SIZE_BITS), 0);

    vmm_set_mapping(KERNEL_PID, (void *)APIC_BASE_VADDR,
                    (void *)__APIC_BASE_PADDR, PG_PREM_RW);
    vmm_set_mapping(KERNEL_PID, (void *)IOAPIC_BASE_VADDR, (void *)ioapic_addr,
                    PG_PREM_RW);
...
void lock_reserved_memory()
{
    ...
    for (unsigned int i = 0; i < map_size; i++)
    {
        ...
        {
            vmm_set_mapping(KERNEL_PID, (pa + (j << PG_SIZE_BITS)),
...
void setup_memory(multiboot_memory_map_t *map, size_t map_size)
{
    ...
    // 首先，标记VGA部分为已占用
    pmm_mark_chunk_occupied(KERNEL_PID, VGA_BUFFER_PADDR >> PG_SIZE_BITS,
                            vga_buf_pgs, 0);
