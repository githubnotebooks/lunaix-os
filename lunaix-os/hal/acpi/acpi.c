...
int acpi_rsdp_validate(acpi_rsdp_t *rsdp);

acpi_rsdp_t *acpi_locate_rsdp(multiboot_info_t *mb_info);
...
int acpi_init(multiboot_info_t *mb_info)
{
    ...
    toc = (acpi_context *)lxcalloc(1, sizeof(acpi_context));
...
acpi_rsdp_t *acpi_locate_rsdp(multiboot_info_t *mb_info)
{
    ...
    uint8_t *mem_start = (uint8_t *)0x4000;
