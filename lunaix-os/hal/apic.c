...
void init_apic()
{
    ...
    kprintf(KINFO "ID: %x, Version: %x, Max LVT: %u\n", apic_id,
            apic_ver & 0xff, (apic_ver >> 16) & 0xff);
...
void temp_intr_routine_rtc_tick(const isr_param *param);

void temp_intr_routine_apic_timer(const isr_param *param);

void test_timer(const isr_param *param);
...
void apic_setup_timer()
{
    ...
    */
    ...
    // TODO: now setup timer with our custom frequency which we can derived from
    // the base frequency
    //          we measured
