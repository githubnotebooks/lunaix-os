...
#define IOAPIC_REG_SEL                                                         \
    *((volatile uint32_t *)(IOAPIC_BASE_VADDR + IOAPIC_IOREGSEL))
#define IOAPIC_REG_WIN                                                         \
    *((volatile uint32_t *)(IOAPIC_BASE_VADDR + IOAPIC_IOWIN))
...
void ioapic_init()
{
    ...
    // physical, APIC ID 0
    ioapic_redirect(ioapic_get_irq(acpi_ctx, PC_AT_IRQ_RTC), RTC_TIMER_IV, 0,
                    IOAPIC_DELMOD_FIXED);
}
...
void ioapic_write(uint8_t sel, uint32_t val)
{
    IOAPIC_REG_SEL = sel;
    IOAPIC_REG_WIN = val;
}

uint32_t ioapic_read(uint8_t sel)
{
    IOAPIC_REG_SEL = sel;
    return IOAPIC_REG_WIN;
}

void ioapic_redirect(uint8_t irq, uint8_t vector, uint8_t dest, uint32_t flags)
{
