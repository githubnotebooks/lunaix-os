...
void _init_page(ptd_t *ptd)
{
    ...
    {
        SET_PTE(ptd, PG_TABLE_IDENTITY, i,
    ...
    {
        SET_PTE(ptd, PG_TABLE_IDENTITY, 256 + i,
    ...
    {
        SET_PDE(ptd, kernel_pde_index + i,
    ...
    {
        SET_PTE(ptd, PG_TABLE_KERNEL, kernel_pte_index + i,
...
void _save_multiboot_info(multiboot_info_t *info, uint8_t *destination)
{
    ...
    current += __save_subset(destination + current, (uint8_t *)info->mmap_addr,
                             info->mmap_length);
    ...
    if (present(info->flags, MULTIBOOT_INFO_DRIVE_INFO))
    {
        ((multiboot_info_t *)destination)->drives_addr =
            (uintptr_t)destination + current;
        current +=
            __save_subset(destination + current, (uint8_t *)info->drives_addr,
                          info->drives_length);
    }
}
